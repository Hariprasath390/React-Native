...
function ProductDetail({ route: { params }, navigation }) {
  ...
  const onSubmitHandler = async (data) => {
    ...
    const availability2 = await response.json();
    setLoading(false);
    setIsAvailable(JSON.parse(availability2));
  };

  console.log(JSON.parse(availability2, console.log("___________________")));
  ...
  const AddToCart = (productname) => {
    ...
    if (
      ...
      if (orderingProduct.length) {
        ...
        if (userInfo) {
          return dispatch(CartDataBaseAdd(orderingProduct[0]?.id, qty))
            .then(() => {
          ...
          return dispatch(addToCart(orderingProduct[0]?.id, qty))
            .then(() => {
